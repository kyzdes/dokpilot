...
- Before planning or editing, read `context-map-dokpilot/context-map.md` and the split files it links to (`known-issues.md`, `decisions.md`, `tasks.md`, `gotchas.md`; `architecture.md` if present).
...
- Never commit `context-map-dokpilot/` content. The skill ensures `.gitignore` excludes it; if you find this folder being staged, remove it from the index and verify the project's `.gitignore` carries the rule.
